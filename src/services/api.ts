...
export const api = {
  ...
  fetchLyrics: async (artist: string, title: string): Promise<string> => {
    ...
    if (!title || !artist) {
      throw new Error(
        `Invalid song data: title="${title}", artist="${artist}"`
      );
    }
    ...
    const artistStr =
      typeof artist === "object"
        ? (artist as any).name || String(artist)
        : String(artist);
    ...
    const cleanedTitle = titleStr
      .replace(
        /\s*\(.*?(Remaster|Remix|Live|Acoustic|Radio Edit|Album Version|Single Version).*?\)/gi,
        ""
      )
      .replace(
        /\s*-\s*(Remaster|Remix|Live|Acoustic|Radio Edit|Album Version|Single Version).*/gi,
        ""
      )
      .trim();
    ...
    try {
      ...
      const lyricsOvhUrl = `https://api.lyrics.ovh/v1/${encodeURIComponent(
        cleanedArtist
      )}/${encodeURIComponent(cleanedTitle)}`;
    ...
    try {
      ...
      const backendUrl = `${API_BASE_URL}/api/lyrics?artist=${encodeURIComponent(
        cleanedArtist
